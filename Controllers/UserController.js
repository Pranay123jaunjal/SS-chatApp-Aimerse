...
exports. register = async (req, res) => {
    try {
        ...
        res.status(500).json({ message: "erro in registering the user ", error: error.message });
    }
};
...
exports. Login = async (req, res) => {
    try {
      ...
      const token = jwt.sign({ userId: user._id, token_user: user.token_user },process.env.JWT_SECRET, {
        expiresIn: '1h', 
      });
 ...
 exports. Logout=async(req, res)=> {
    try {
        ...
        const latestLogin = await UserLoginHistory.findOne({ token_user, status: "active" }).sort({ createdAt: -1 });

        if (!latestLogin) {
            return res.status(400).json({ success: false, message: "No active session found invalid token" });
        }
